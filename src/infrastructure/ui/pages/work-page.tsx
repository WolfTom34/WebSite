                ...
                {FLEET.map((unit) => (
                    <div key={unit.id} className="project-card">
                        <div className="card-header">
                            ...
                            <div className={`status-dot ${unit.status === 'ACTIVE' ? 'on' : 'off'}`}></div>
                        </div>
                        <h2 className="fleet-name">{unit.name}</h2>
                        <p>{unit.description}</p>
                        <div className="tech-stack">
                            {unit.capabilities.map(cap => (
                                <span key={cap} className="tech-tag">{cap}</span>
                            ))}
                        </div>
                        <div className="unit-footer">
                            <span style={{ color: unit.status === 'ACTIVE' ? '#00FF41' : '#FFD700' }}>
                                {unit.status === 'ACTIVE' ? t('fleet.status_active') : t('fleet.status_prototype')}
                            </span>
                        </div>
                        <button
                            className="requisition-btn"
                            onClick={() => openTransmission(`${t('cta.requisition')}: ${unit.designation}`)}
                        >
                            {t('cta.requisition')}
                        </button>
                    </div>
